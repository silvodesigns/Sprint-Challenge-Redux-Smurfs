...
`Actions describe interactions that  happen in the interface , reducer is how we respond to the actions and the store is where ultimately we will see the changes in our data as results of the actions`
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
`Aplication state is the one single source of data for our entire application while component state could be pieces that we need from the application data that are important to a single component only ` 
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

`redux- thunk are functions that are returned by other functions that allow us to perform asyn operation on react`
